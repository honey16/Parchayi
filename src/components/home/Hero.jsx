...
const Hero = () => {
  ...
  return (
    <div className="relative w-screen min-h-screen overflow-hidden">
      ...
      <div className="relative flex w-full h-full">
        {/* Left section - text content */}
        <div className="md: w-[70%] text-white p-24 pl-32">
          <h1 className="sm:text-4xl md:text-5xl lg:text-6xl font-serif mb-8 leading-tight">
            Your one-stop
            <br />
            solution for
            <br />
          ...
          <div className="flex gap-14">
            {stats.map((stat, index) => (
              <div key={index}>
                <div className="md:text-4xl lg:text-7xl font-serif text-orange">{stat.number}</div>
                <div className="text-base text-gray-400 mt-2">{stat.label}</div>
              </div>
            ))}
          </div>
        ...
        {/* Right section - images */}
        <div className="sm:hidden md:block w-[30%] relative">
          <div className="absolute right-0 md:top-[45%] lg:top-[20%] lg2:-left-[600px] lg2:top-[20%]">
            <div className="flex gap-14">
              {images.map((image, index) => (
                <div key={index} className="md:w-[150px] md:last:mr-[80px] lg:w-[175px] lg:last:mr-[100px] lg2:w-[300px] last:mr-[-200px]">
                  <div className="relative">
                    <img
                      src={image.src}
                      alt={`Photography ${index + 1}`}
                      className="w-full md:h-[400px] lg:h-[600px] object-cover rounded-lg shadow-lg"
                    />
                    <div className="mt-4 flex justify-between items-center">
                      <p className="text-gray-800 text-lg font-medium">{image.name}</p>
                      <p className="text-gray-600 text-lg">{image.year}</p>
